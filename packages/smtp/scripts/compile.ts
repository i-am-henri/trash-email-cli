...
async function compileTypeScript(code: string) {
	...
	return result.code.replace("process.env.AUTH_TOKEN", "");
}
		...
		if (statSync(filePath).isDirectory()) {
			...
			for await (const subfolderFile of subfolder) {
				...
				if (subfolderFile === "plugin.ts") {
					const pluginFile = (
						await readFile(`./${filePath}/plugin.ts`)
					).toString();
...
async function savePlugins(plugins: CompilePlugin[]) {
	...
	for (const plugin of plugins) {
		...
		consola.success(
			`Saved the plugin ${plugin.name} successfully! Please insert now ${plugin.name} into the plugins file (/config/plugins) in the right order!`,
		);
	}
}
